...
def create_quote_content() -> tuple[str, str, str]:
    ...
    for attempt in range(MAX_ATTEMPTS):
        ...
        # --- NEW PROMPT FOCUSED ON DOG FACTS ---
        master_prompt = f"""
...
def generate_video_with_music(part1: str, part2: str, output_filename: str):
    ...
    # Load your outro clip to get its duration
    try:
        outro_clip_path = "like_subscribe.mp4" # Assuming it's in video_temp
        outro_clip_for_duration = VideoFileClip(outro_clip_path)
        OUTRO_DURATION = outro_clip_for_duration.duration
        outro_clip_for_duration.close() # Close the clip after getting duration
    except Exception as e:
        exit(f"❌ ERROR: Could not find 'like_subscribe.mp4' in 'video_temp' folder. Details: {e}")
    ...
    try:
        ...
        background_video_folder = 'dogs_temp'
        available_videos = sorted([f for f in os.listdir(background_video_folder) if f.endswith(('.mp4', '.mov')) and f != 'like_subscribe.mp4'])
        if not available_videos: exit(f"❌ ERROR: No background videos found in '{background_video_folder}'.")
        # (Your sequential logic for picking a video is here and will work as before)
        state_file = 'dogs_temp/last_video_index.txt'
        last_index = -1
        if os.path.exists(state_file):
            with open(state_file, 'r') as f: last_index = int(f.read() or -1)
    ...
    # Load music for the TOTAL duration
    music_clip = AudioFileClip(chosen_music_path).set_duration(TOTAL_DURATION)
    
    # Prepare the main content video background
    ...
    # --- 4. Create Main Content Layers (Heading and Quotes) ---
    print(" Adding text layers...")
    heading_text = "Dogs Facts"
    box_width = int(1080 * 0.7)
    box_height = 110
    final_position = ('center', int(1920 * 0.20))
    heading_bg = ColorClip(size=(box_width, box_height), color=(255, 255, 255)).set_position(final_position).set_duration(MAIN_VIDEO_DURATION)
    heading_clip = TextClip(heading_text, fontsize=75, color='black', font='Arial-Bold', size=heading_bg.size).set_position(final_position).set_duration(MAIN_VIDEO_DURATION)
    ...
    quote_clip1 = TextClip(part1, fontsize=80, color='white', font='Arial-Bold', stroke_color='black', stroke_width=3,
                           size=(1080 * 0.9, None), method='caption').set_position('center').set_duration(part1_duration).fx(vfx.fadein, 1).fx(vfx.fadeout, 0.5)
    quote_clip2 = TextClip(part2, fontsize=80, color='white', font='Arial-Bold', stroke_color='black', stroke_width=3,
                           size=(1080 * 0.9, None), method='caption').set_position('center').set_start(part2_start_time).set_duration(part2_duration).fx(vfx.fadein, 0.5)

    # --- 5. Composite the Main Video ---
    main_video = CompositeVideoClip(
        [main_background, heading_bg, heading_clip, quote_clip1, quote_clip2]
    ).set_duration(MAIN_VIDEO_DURATION)

    # --- 6. Load the Outro Clip ---
    print(" Adding 'Like & Subscribe' outro...")
    outro_clip = VideoFileClip(outro_clip_path).resize(height=1920).crop(x_center=background_clip.w/2, width=1080)
    
    # --- 7. Concatenate Main Video and Outro ---
    final_video = concatenate_videoclips([main_video, outro_clip])
    
    # Set the continuous music across the entire final video
    ...
    if choice == '1' or choice == '2':
        ...
        if choice == '2':
            ...
            try:
                ...
                initial_tags = ["dogs", "facts", "shorts","ytshorts"]
